...
public final class DefaultInboxQueue extends AbstractInboxQueue<ByteChannel> {
     ...
     */
    public DefaultInboxQueue(final @Nonnull InternalKnxClient internalClient, final @Nonnull SelectableChannel channel) {
        super(internalClient, channel);
    }

    @Override
    protected void receive(final @Nonnull ByteChannel channel, final @Nonnull ByteBuffer bb) throws IOException {
        channel.read(bb);
    }
}
