 ...
 * @author PITSCHR
 */
public class KnxMainWriteAsync extends AbstractKnxMain {
    private static final Logger LOG = LoggerFactory.getLogger(KnxMainWriteAsync.class);
        ...
        try (final DefaultKnxClient client = new DefaultKnxClient(routerAddress)) {
            ...
            for (final String value : values) {
                ...
                for (final GroupAddress groupAddress : DEFAULT_GROUP_ADDRESSES) {
                    Future<TunnellingAckBody> future = client.writeRequestAsync(groupAddress, dpValue);
