...
/**
 * Extension to start/stop the {@link MockHttpDaemon2} (and {@link MockServer} indirectly).
 * <p/>
 * It will be invoked using {@link MockDaemonTest} annotation.
 *
 * @author PITSCHR
 */
public final class MockDaemonTestExtension2
        implements ParameterResolver, BeforeTestExecutionCallback, AfterTestExecutionCallback {
    private static final Logger log = LoggerFactory.getLogger(MockDaemonTestExtension2.class);
    private static final Map<ExtensionContext, MockServer> mockServers = new ConcurrentHashMap<>();
    private static final Map<ExtensionContext, MockHttpDaemon2> mockDaemons = new ConcurrentHashMap<>();
    ...
    /**
     * Initializes the {@link MockHttpDaemon2} and start
     *
     * @param context
     */
    @Override
    public void beforeTestExecution(final ExtensionContext context) {
        ...
        if (!mockServers.containsKey(context)) {
            ...
            final var annotation = AnnotationSupport.findAnnotation(context.getRequiredTestMethod(), MockDaemonTest2.class).get();
            // -------------------------------------
            // 1) start mock server
            // -------------------------------------
            ...
            // -------------------------------------
            stopwatch.reset();
            final var mockHttpDaemon = new MockHttpDaemon2();
    ...
    @Override
    public MockHttpDaemon2 resolveParameter(final ParameterContext paramContext, final ExtensionContext context) throws ParameterResolutionException {
    ...
    public boolean supportsParameter(final ParameterContext paramContext, final ExtensionContext context) throws ParameterResolutionException {
        return paramContext.getParameter().getType().equals(MockHttpDaemon2.class);
    }
}
